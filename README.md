...
- 退回证据为聊天截图或电话录音至少一项；
- 退回申诉期从成功领取起连续计算 48 小时；人工电话确认有效时及时结算，否则满48小时无正式退回申请时自动认定有效并结算；
...
系统管理员进入管理后台「资金 → 客资固定积分」，一次保存两个数值：运营客资领取积分为正整数，加盟商供客积分为非负整数（0 表示不发放）。仅系统管理员可读取、修改该配置。首次保存前沿用已有积分规则，不预设实际业务金额。

保存后，后台手工录入和飞书导入客资按固定领取积分派发，加盟商领取时扣除派发单保存的金额；其他来源的领取价格继续使用原有规则。加盟商供客积分在被领取时锁定：人工电话确认有效时及时结算；未提前确认的，领取后连续 48 小时未正式申请退回时自动认定有效并结算入账。退回草稿不会冻结奖励；正式退回申请冻结未入账奖励。已入账奖励不影响领取方48小时内申请退回：退回审核通过时系统幂等冲回，驳回时保持已入账，重复审核不会重复冲回。存量 `WAITING_CLAIM` / `OBSERVING` 奖励依新规则继续处理且不重算金额；到期扫描不重复处理已结算记录，退回批准仍按上述规则幂等冲回。详见 [9.13 变更说明](docs/requirements/2026-09-13-领取48小时自动有效与奖励结算.md)。

配置复用现有版本化 `SystemConfig`，无需数据库结构迁移。接口、并发版本冲突与验证说明见 [9.12 需求及接口说明](docs/requirements/2026-09-12-固定积分及反馈整改.md)。API 与 Scheduler 应使用同一版本；Scheduler 每 5 分钟分批标记超过 48 小时仍未提交的退回草稿，运营默认待审核列表只显示已正式提交的申诉。
