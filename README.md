...
- 退回证据为聊天截图或电话录音至少一项；
- 退回申诉可用时长从成功领取起计算 48 小时，正式提交退回后暂停，驳回后恢复剩余时长；人工电话确认有效时及时结算，否则可用时长用尽且无审核中退回时自动认定有效并结算；
- V1.2 仅支持平台人工派发，不提供自动、随机、轮询、权重或抢单；
- 禁止供应商将自己提供的客资派发给自己；
- 正式退回申请在审核期间冻结未入账奖励；通过时取消未入账奖励或幂等冲回已入账奖励，驳回时保持入账或恢复到期结算；异常冲正单独审计。

V1.2 不包含在线支付、自动派发、微信小程序、云外呼或 H5 自动录音。

## 客资固定积分配置

系统管理员进入管理后台「资金 → 客资固定积分」，一次保存两个数值：客资领取积分为正整数，加盟商供客积分为非负整数（0 表示不发放）。仅系统管理员可读取、修改该配置。首次保存前沿用已有积分规则，不预设实际业务金额。

保存后，所有来源客资的领取积分立即生效，包括已经派发但尚未领取的客资。待领取列表、详情、预占积分和余额判断使用当前配置；领取事务按最新配置实际扣分，并保存本次金额和配置版本。已领取记录、重复领取请求、退款及历史统计保留实际扣费金额，不随之后改价重算。页面发现价格变化时先提示并刷新，用户按最新价格确认后再扣费。接口仍保留 `operation_claim_points` 字段以兼容已有配置，含义已扩展为所有来源的客资领取积分。详见 [9.16 实时领取积分规则及接口说明](docs/requirements/2026-09-16-领取积分实时生效.md)。

加盟商供客积分仍在被领取时锁定，独立于领取积分设置：人工电话确认有效时及时结算；未提前确认的，领取后连续 48 小时未正式申请退回时自动认定有效并结算入账。退回草稿不会冻结奖励；正式退回申请冻结未入账奖励。已入账奖励不影响领取方48小时内申请退回：退回审核通过时系统幂等冲回，驳回时保持已入账，重复审核不会重复冲回。存量 `WAITING_CLAIM` / `OBSERVING` 奖励依新规则继续处理且不重算金额；到期扫描不重复处理已结算记录，退回批准仍按上述规则幂等冲回。详见 [9.13 变更说明](docs/requirements/2026-09-13-领取48小时自动有效与奖励结算.md)。

积分配置复用现有版本化 `SystemConfig`，此项无需数据库结构迁移。本次与 9.15/9.16 反馈合并发布，退回暂停计时另需执行 `0022_return_clock_pause` 迁移，详见 [9.15 反馈整改](docs/requirements/2026-09-16-九月十五日反馈整改.md)。接口、并发版本冲突与验证说明见 [9.12 需求及接口说明](docs/requirements/2026-09-12-固定积分及反馈整改.md)。API 与 Scheduler 应使用同一版本；Scheduler 每 5 分钟分批标记超过 48 小时仍未提交的退回草稿，运营默认待审核列表只显示已正式提交的申诉。
...
新建和编辑加盟商服务区域均支持「全选全国城市」和清空已选区域；全国选择展开当前目录的 341 个城市，主要城市仍可单独指定。创建及修改服务区域接口最多接收 1000 个区域编码，保存后继续按现有城市、区县和乡镇覆盖规则匹配。
...
飞书是显式可选集成：启用时设置 `FEISHU_ENABLED=true`，配置应用、数据表和“客户视图”，并保持 `FEISHU_WRITEBACK_ENABLED=false`。一期仅允许运营在公海池手动触发单向导入，不执行定时同步或飞书回写；不启用时不得保留无效生产凭据。公海池按系统派生的“运营录入/加盟商提供”区分客户来源，加盟商供客在当地没有其他合格接收方时暂存。运营可手动重新匹配；历史公海定时重匹配默认关闭，须先预览受影响记录，再显式设置 `PUBLIC_POOL_AUTO_REMATCH_ENABLED=1` 启用。重匹配仅转入待派发池，不自动派发、不扣积分。
