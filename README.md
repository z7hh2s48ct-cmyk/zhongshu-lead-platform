...
保存后，后台手工录入和飞书导入客资按固定领取积分派发，加盟商领取时扣除派发单保存的金额；其他来源的领取价格继续使用原有规则。加盟商供客积分在被领取时锁定，审核通过不会增加余额；电话确认有效后观察 3 个工作日，到期且无待处理退回才结算。正式退回冻结供客积分，退回成立取消、不成立恢复观察。历史派发及已生成供客积分不随配置变化重算，48 小时退回期限保持不变。
