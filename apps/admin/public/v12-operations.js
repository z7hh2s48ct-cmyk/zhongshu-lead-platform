...
async function supplementOperationLead(lead){
  ...
  try{
    ...
    await openPlatformLeadForm(lead,true,{verificationInfo:detail?.verification_info||null,refresh:review,forcePublicPool:lead.source_kind==='FEISHU_IMPORT'});
  }catch(error){toast(error.message,true)}
}
...
async function dispatch(){
  ...
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>待人工派发池</h2><p>运营可核对完整手机号、客资提供方和录入人员；派发时必须选择具体加盟商员工。</p></div></div>${table(['客户','所在地','客资来源','提供方 / 录入人','客户需求','操作'],rows)}${pager(d)}</section>`);
...
async function returnDetail(id){
  ...
  modal('退回申诉详情',`<div class="ops-detail-grid">${[['退回编号',recordCode(x.id,'TH')],['派发编号',recordCode(x.assignment_id,'PF')],['客户',x.customer_name],['完整电话',x.phone||x.phone_masked],['处理状态',label(x.status)],['退回原因',label(x.reason_code)],['申诉人',x.submitted_by_name],['核验人员',telesalesName(verification.assignee_user_id)],['核验状态',label(verification.status)],['联系结果',label(verification.contact_result)],['核验结论',label(verification.conclusion)],['申诉截止',fmt(x.appeal_deadline_at)],['资金影响',fundImpact],['供资奖励',reward.status?label(reward.status):'无关联奖励'],['终审说明',x.final_decision_reason]].map(([a,b])=>`<div class="ops-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><section class="ops-card"><h3>申诉说明</h3><p>${esc(x.description||'暂无说明')}</p></section><section class="ops-card"><h3>核验备注</h3><p>${esc(verification.note||'暂无核验备注')}</p></section><section class="ops-card"><h3>申诉证据</h3><div class="ops-detail-grid">${evidenceList(x.evidences)}</div></section>${canFinalReview?`<div class="ops-actions">${finalActions}</div>`:x.status==='REVIEWING'?'<div class="ops-notice">等待电销提交事实核验结论后，才能进行运营终审。</div>':''}`,()=>document.querySelectorAll('[data-final]').forEach(b=>b.onclick=()=>finalReview(id,b.dataset.final)));
}
...
async function taskDetail(id){
  ...
  const actions=`<div class="ops-actions"><button class="ops-btn" data-detail-assign="${esc(id)}">${x.assignee_user_id?'重新分配核验人员':'分配核验人员'}</button>${r.status==='REVIEWING'?finalAction:''}</div>`;
  modal('电话核验详情',`<div class="ops-detail-grid">${[['客户',lead.customer_name],['联系电话',lead.phone||lead.phone_masked],['所在地',`${lead.city||''} ${lead.district||''}`],['任务状态',verificationTaskLabel(x)],['核验人员',telesalesName(x.assignee_user_id)],['退回原因',label(r.reason_code)],['证据数量',`${evidenceTotal} 份`],['核验截止',fmt(x.due_at)],['申诉截止',fmt(r.appeal_deadline_at)],['联系结果',label(x.contact_result)],['核验结论',label(x.conclusion)],['核验提交人',info.submitted_by_name],['核验提交时间',fmt(info.submitted_at)]].map(([a,b])=>`<div class="ops-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><section class="ops-card"><h3>申诉说明</h3><p>${esc(r.description||'暂无说明')}</p></section><section class="ops-card"><h3>核验备注</h3><p>${esc(info.note||'暂无核验备注')}</p></section>${actions}`,()=>{
