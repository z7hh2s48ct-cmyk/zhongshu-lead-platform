...
function syncRouteFromUrl({canonicalize=false}={}){
  ...
  if((S.view==='supplements')!==(nextView==='supplements'))resetLeadReportFilters(nextView);
  S.view=nextView;
...
async function publicPool(){
  ...
  document.querySelectorAll('[data-public-pool-delete]').forEach(button=>button.onclick=()=>deleteOwnOperationLead(byId[button.dataset.publicPoolDelete],publicPool));
}
...
async function review(){
  ...
  const rows=leads.map(lead=>{
    ...
    if(!readOnly&&canDeleteOwnOperationLead(lead))actions.push(`<button class="ops-btn danger" data-operation-lead-delete="${esc(lead.id)}">删除本人客资</button>`);
  ...
  const queueTitle=supplementQueue?'待运营补充':readOnly?'客资总览':'客资录入与流转';
  const queueDescription=supplementQueue?'集中处理运营选择待补充且尚未处理完成的客资，包含以前进入待补充的记录。保存后重新校验，完整时进入派发池，仍有缺项时继续保留；可点击重置查看全部待补充。':'统一展示平台客资与加盟商客资队列；日期始终按客资创建时间筛选，客资状态与派发状态分开。“派发运营人员”是实际执行派发的后台人员。加盟商客资在当地暂无其他合格接收方时先进入公海池；缺少可派发地区时再分配电销核实。';
  const pendingControl=supplementQueue?'<div class="ops-notice">已固定筛选：待运营补充</div>':`<select class="ops-input" id="lead-pending-reason-filter"><option value="">全部待办事项</option><option value="PRE_DISPATCH_REWORK_REQUIRED" ${S.leadPendingReason==='PRE_DISPATCH_REWORK_REQUIRED'?'selected':''}>待运营补充</option></select>`;
  ...
  const queue=`<section class="ops-card"><div class="ops-card-head"><div><h2>客资记录</h2><p>列表显示提供加盟商、实际录入人、当前接收方和加盟商跟进人；完整派发与人员变更记录请在详情中查看。</p></div></div>${table(['客户','来源','提供加盟商 / 录入人','所在地','当前接收方','加盟商跟进人','客资状态','派发状态','派发运营人员','最新加盟商跟进','创建时间','操作'],rows)}${pager(reportData)}</section>`;
  ...
  document.querySelectorAll('[data-operation-lead-delete]').forEach(button=>button.onclick=()=>deleteOwnOperationLead(leads.find(lead=>lead.id===button.dataset.operationLeadDelete),review));
...
async function telesales(){
  ...
  const rows=(data.items||[]).map(task=>{
    ...
    return `<tr><td><b>${esc(lead.customer_name||'待核验客户')}</b><br><small>${esc(lead.phone||lead.phone_masked||'--')}</small></td><td>${badge(lead.source_kind)}</td><td>${verificationTaskBadge(task)}</td><td>${esc(telesalesName(task.assignee_user_id))}</td><td>${esc(label(task.conclusion))}</td><td>${esc(nextStep)}</td><td>${fmt(task.due_at)}</td><td>${action}</td></tr>`;
  });
...
async function dispatch(){
  ...
  const rows=(d.items||[]).map(x=>{
    ...
    return `<tr><td><b>${esc(x.customer_name)}</b><br>${esc(x.phone||x.phone_masked||'--')}</td><td>${esc(x.city||'--')} ${esc(x.district||'')}</td><td>${esc(label(x.source_kind))}</td><td>${esc(x.supplier_company_name||'平台客资')}<br><small>${esc(x.submitter_name||'未知录入人')}</small></td><td>${esc(x.need_summary||'--')}</td><td><div class="ops-actions">${actions.join(' ')}${verificationFlag}</div></td></tr>`;
  });
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>待人工派发池</h2><p>运营可核对完整手机号、客资提供方和录入人员；有在职员工时选择具体员工；无在职员工时自动派发给负责人。</p></div></div>${table(['客户','所在地','客资来源','提供方 / 录入人','客户需求','操作'],rows)}${pager(d)}</section>`);
...
async function finance(){
  const [financeDashboard,companyPage,activePackages,allPackages,priceRules,ledgerPage,rewardPage,currentRewardRule,cities,leadPointsSettings]=await Promise.all([
