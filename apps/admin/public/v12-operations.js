...
Object.assign(L,{NOT_APPLICABLE:'无需供资奖励'});
...
Object.assign(AUDIT_RESOURCE_LABEL,{lead_points_settings:'客资固定积分'});
...
const badge=(v,text=label(v))=>`<span class="ops-status ${['APPROVED','SETTLED','CLAIMED'].includes(v)?'ok':['REJECTED','CANCELLED','REVERSED'].includes(v)?'bad':'warn'}">${esc(text)}</span>`;
...
async function review(){
  ...
  const [reportData,operationUsers,filterOptions,exportTasks,supplementHistory]=await Promise.all([
    ...
    supplementQueue?api(`/v1.2/admin/leads/pre-dispatch-rework-history${qs({page:S.supplementHistoryPage,page_size:20})}`):Promise.resolve({items:[],total:0,page:1,page_size:20}),
  ]);
  ...
  const rows=leads.map((lead,index)=>{
    ...
    if(!readOnly&&closedQueue&&lead.lead_status==='CLOSED')actions.push(`<button class="ops-btn primary" data-lead-reopen="${esc(lead.id)}">重新启用并选择接收方</button>`);
  ...
  const queueDescription=supplementQueue?'集中处理当前待补充客资，并在下方找回所有曾被标记为待运营补充的历史记录。保存后重新校验，完整时进入派发池，仍有缺项时继续保留。':closedQueue?'统一查看已关闭客资。运营填写重新启用原因后，系统保留旧派发和积分历史，再进入接收加盟商选择；新派发单领取后重新计算 48 小时。':'统一展示平台客资与加盟商客资队列；日期始终按客资创建时间筛选，客资状态与派发状态分开。“派发运营人员”是实际执行派发的后台人员。加盟商客资在当地暂无其他合格接收方时先进入公海池；缺少可派发地区时再分配电销核实。';
  ...
  const queue=`<section class="ops-card"><div class="ops-card-head"><div><h2>客资记录</h2><p>列表显示提供加盟商、实际录入人、当前接收方、供资奖励和加盟商跟进人；完整派发与人员变更记录请在详情中查看。</p></div></div>${table(['序号','客户','来源','提供加盟商 / 录入人','所在地','当前接收方','加盟商跟进人','客资状态','派发状态','供资奖励','派发运营人员','最新加盟商跟进','创建时间','操作'],rows)}${pager(reportData)}</section>`;
  ...
  const historyPanel=supplementQueue?`<section class="ops-card"><div class="ops-card-head"><div><h2>历史待补充记录</h2><p>依据历次运营处置审计找回，不受当前待办标记变化影响；已进入后续流程的记录保留查阅，不会回退。</p></div></div>${table(['序号','客户','当前状态','待补充状态','操作'],historyRows)}${historyPager}</section>`:'';
  ...
  document.querySelector('#lead-report-filter').onsubmit=event=>{event.preventDefault();S.leadCreatedFrom=document.querySelector('#lead-created-from').value;S.leadCreatedTo=document.querySelector('#lead-created-to').value;S.leadSource=document.querySelector('#lead-source-filter').value;S.leadSupplierCompanyId=document.querySelector('#lead-supplier-company-filter').value;S.leadSubmitterId=document.querySelector('#lead-submitter-filter').value;S.leadPendingReason=supplementQueue?'PRE_DISPATCH_REWORK_REQUIRED':closedQueue?'':document.querySelector('#lead-pending-reason-filter').value;S.leadPhone=document.querySelector('#lead-phone-filter').value.trim();S.leadRegion=document.querySelector('#lead-region-filter').value.trim();S.leadReceiverCompanyId=document.querySelector('#lead-receiver-filter').value;S.leadStatusFilter=closedQueue?'CLOSED':document.querySelector('#lead-status-filter').value;S.assignmentStatusFilter=document.querySelector('#assignment-status-filter').value;S.leadAssignerId=document.querySelector('#lead-assigner-filter').value;S.page=1;review()};
...
async function telesales(){
  ...
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>前置电销核验</h2><p>运营在此分配或改派；平台来源补充资料由运营处理，加盟商来源才可退回加盟商补正。超时任务需改派后重新核验。</p></div></div>${table(['序号','客户','来源','任务状态','电销人员','事实结论','下一步','核验截止','操作'],rows)}${pager(data)}</section>`);
...
function bindServiceRegionBuilder(prefix,cities,initialAreas=[]){
  ...
  let primaryCityCode=cityForRegion(selected.get(initialPrimaryArea?.region_code))?.code||'';
...
async function dispatch(){
  ...
  shell(`<section class="ops-card"><div class="ops-card-head"><div><h2>待人工派发池</h2><p>运营可核对完整手机号、客资提供方和录入人员；有在职员工时选择具体员工；无在职员工时自动派发给负责人。</p></div></div>${table(['序号','客户','所在地','客资来源','提供方 / 录入人','客户需求','操作'],rows)}${pager(d)}</section>`);
...
async function returnDetail(id){
  ...
  const fundImpact=!x.submitted_at?'尚未正式提交，不产生退款或结算冻结':x.status==='APPROVED'?`已返还 ${Number(x.refund_points||0)} 积分，客资已关闭`:'终审支持退回后按原领取流水返还积分';
  ...
  modal('退回申诉详情',`<div class="ops-detail-grid">${[['退回编号',recordCode(x.id,'TH')],['派发编号',recordCode(x.assignment_id,'PF')],['客户',x.customer_name],['完整电话',x.phone||x.phone_masked],['处理状态',returnStatusLabel(x.status)],['申诉时间',x.submitted_at?fmt(x.submitted_at):'尚未提交'],['退回原因',label(x.reason_code)],['申诉人',x.submitted_by_name],['核验人员',telesalesName(verification.assignee_user_id)],['核验状态',label(verification.status)],['联系结果',label(verification.contact_result)],['核验结论',label(verification.conclusion)],['申诉截止',fmt(x.appeal_deadline_at)],['资金影响',fundImpact],['供资奖励',reward.status?label(reward.status):'无关联奖励'],['终审说明',x.final_decision_reason]].map(([a,b])=>`<div class="ops-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><section class="ops-card"><h3>申诉说明</h3><p>${esc(x.description||'暂无说明')}</p></section><section class="ops-card"><h3>核验备注</h3><p>${esc(verification.note||'暂无核验备注')}</p></section><section class="ops-card"><h3>申诉证据</h3><div class="ops-detail-grid">${evidenceList(x.evidences)}</div></section><section class="ops-card"><h3>电销核验证据</h3><div class="ops-detail-grid">${evidenceList(verification.evidences)}</div></section>${pathActions}${canFinalReview?`<div class="ops-actions">${finalActions}</div>`:x.status==='REVIEWING'?'<div class="ops-notice">等待电销提交事实核验结论后，才能进行运营终审。</div>':''}`,()=>{
...
async function taskDetail(id){
  ...
  modal('电话核验详情',`<div class="ops-detail-grid">${[['客户',lead.customer_name],['联系电话',lead.phone||lead.phone_masked],['所在地',`${lead.city||''} ${lead.district||''}`],['任务状态',verificationTaskLabel(x)],['核验人员',telesalesName(x.assignee_user_id)],['退回原因',label(r.reason_code)],['证据数量',`${evidenceTotal} 份`],['核验截止',fmt(x.due_at)],['申诉截止',fmt(r.appeal_deadline_at)],['联系结果',label(x.contact_result)],['核验结论',label(x.conclusion)],['核验提交人',info.submitted_by_name],['核验提交时间',fmt(info.submitted_at)]].map(([a,b])=>`<div class="ops-detail"><small>${a}</small><b>${esc(b||'--')}</b></div>`).join('')}</div><section class="ops-card"><h3>申诉说明</h3><p>${esc(r.description||'暂无说明')}</p></section><section class="ops-card"><h3>核验备注</h3><p>${esc(info.note||'暂无核验备注')}</p></section><section class="ops-card"><h3>电销核验证据</h3><div class="ops-detail-grid">${evidenceList(info.evidences)}</div></section>${actions}`,()=>{
...
function bindLeadPointsSettings(settings){
  ...
  form.onsubmit=async event=>{
    ...
    try{
      ...
      document.querySelector('#lead-points-message').textContent='固定积分已保存；已派发客资及已生成的供客积分记录继续按原金额执行。';
      toast('固定积分已保存');
    }catch(error){toast(error.message,true)}finally{submit.disabled=false}
  };
}
async function economics(){
  const pointFlows=await api(`/v1.2/reports/point-flows${qs({period:S.pointFlowPeriod,anchor:S.pointFlowAnchor,page:S.pointFlowPage,page_size:20})}`);
  S.pointFlows=pointFlows;
  shell(pointFlowSection(pointFlows));
  bindPointFlowControls(economics);
}
async function finance(){
  const [financeDashboard,pointFlows,companyPage,activePackages,allPackages,priceRules,ledgerPage,rewardPage,currentRewardRule,cities,leadPointsSettings]=await Promise.all([
