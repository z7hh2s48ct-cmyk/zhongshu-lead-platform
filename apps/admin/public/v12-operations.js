...
async function chooseDispatchEmployee(companyId,companyName,{returnReceiverOverride=false,onSubmit}){
  try{
    ...
    if(!employees.length){toast('该加盟商暂无可接收客资的在职员工',true);return}
    const options=employees.map(user=>`<option value="${esc(user.id)}">${esc(user.display_name||user.username||'未命名员工')}</option>`).join('');
    modal(returnReceiverOverride?'确认例外直派':'选择接收员工',`<form class="ops-form" id="direct-dispatch-form"><div class="ops-notice">客资将直接派发给 ${esc(companyName||'所选加盟商')} 的指定员工，负责人仅接收通知并查看进度。</div><div class="ops-field"><label for="direct-dispatch-employee">接收员工 *</label><select class="ops-input" id="direct-dispatch-employee">${options}</select></div><div class="ops-field"><label for="direct-dispatch-note">${returnReceiverOverride?'例外派发原因 *':'派发备注'}</label><textarea class="ops-textarea" id="direct-dispatch-note" ${returnReceiverOverride?'required minlength="2"':''}></textarea></div><div class="ops-actions"><button class="ops-btn" type="button" id="direct-dispatch-cancel">取消</button><button class="ops-btn primary" id="direct-dispatch-submit">确认直派</button></div></form>`,()=>{
      ...
      form.onsubmit=async event=>{event.preventDefault();const employeeUserId=document.querySelector('#direct-dispatch-employee').value,note=document.querySelector('#direct-dispatch-note').value.trim();if(!employeeUserId){toast('请选择接收员工',true);return}if(returnReceiverOverride&&note.length<2){toast('请至少填写 2 个字的例外派发原因',true);return}submit.disabled=true;try{await onSubmit(employeeUserId,note);closeModal()}catch(error){submit.disabled=false;toast(error.message,true)}};
    });
  }catch(error){toast(error.message,true)}
}
function dispatchOne(leadId,companyId,returnReceiverOverride=false,companyName=''){chooseDispatchEmployee(companyId,companyName,{returnReceiverOverride,onSubmit:async(employeeUserId,note)=>{await api(`/v1.2/dispatch-pool/${encodeURIComponent(leadId)}/dispatch`,{method:'POST',body:JSON.stringify({company_id:companyId,employee_user_id:employeeUserId,idempotency_key:`dispatch-${crypto.randomUUID()}`,note:returnReceiverOverride?null:note||null,return_receiver_override:returnReceiverOverride,return_receiver_override_reason:returnReceiverOverride?note:null})});toast('客资已直派给所选员工');await dispatch()}})}
