...
async function review(){
  ...
  const queueDescription=supplementQueue?'集中处理电销标记为资料待补的客资；保存后系统会立即重新校验，资料完整时进入派发池，仍有缺项时继续保留在本栏目。':'统一展示平台客资与加盟商客资队列；日期始终按客资创建时间筛选，客资状态与派发状态分开。“派发运营人员”是实际执行派发的后台人员。加盟商客资在当地暂无其他合格接收方时先进入公海池；缺少可派发地区时再分配电销核实。';
  ...
  document.querySelector('#lead-filter-reset').onclick=()=>{S.leadCreatedFrom='';S.leadCreatedTo='';S.leadSource='';S.leadSupplierCompanyId='';S.leadSubmitterId='';S.leadPendingReason=supplementQueue?'PRE_DISPATCH_REWORK_REQUIRED':'';S.leadPhone='';S.leadRegion='';S.leadReceiverCompanyId='';S.leadStatusFilter='';S.assignmentStatusFilter='';S.leadAssignerId='';S.page=1;review()};
...
async function returnDetail(id){
  ...
  const fundImpact=x.status==='APPROVED'?`已返还 ${Number(x.refund_points||0)} 积分，客资已关闭`:'终审支持退回后按原领取流水返还积分';
