...
async function savePlatformLead(item,correction=false,refresh=null,forcePublicPool=false){
  ...
  if(!payload)return;
  if(correction&&id){Object.assign(payload,{phone:item?.phone||null,source_channel:item?.source_channel||null,source_detail:item?.source_detail||null,category_code:item?.category_code||null,budget_min:item?.budget_min??null,budget_max:item?.budget_max??null,consent_confirmed:Boolean(item?.consent_confirmed)})}
  try{
