...
const S={me:null,view:'home',id:'',page:1,unreadNotifications:0,supplyTermination:null};
const LABEL={DRAFT:'待完善',PENDING:'审核中',PENDING_REVIEW:'平台审核中',PUBLIC_POOL:'等待当地接收方',READY_DISPATCH:'已进入派发',DUPLICATE:'重复信息复核中',PENDING_CLAIM:'待领取',CLAIMED:'已领取',FOLLOWING:'跟进中',RETURN_PENDING:'退回处理中',RETURNED:'已退回',RELEASED:'已释放',EXPIRED:'已过期',CLOSED:'已关闭',COMPLETED:'已完成',UNCONTACTED:'未联系',CONTACTED:'已联系',INTERESTED:'有意向',NOT_INTERESTED:'无意向',DEAL:'电话确认有效',INVALID:'需要修改',SUBMITTED:'已提交',VERIFYING:'核验中',REVIEWING:'待终审',NEED_MORE_EVIDENCE:'待补证',APPROVED:'审核通过',REJECTED:'需要修改',CLEAR:'未发现重复',HARD_DUPLICATE:'近期已有相同客户',REWARD_DUPLICATE:'已有相同客户记录',HISTORICAL_SUSPECT:'历史记录待确认',OVERRIDDEN:'已人工确认',OBSERVING:'待自动结算',FROZEN:'暂缓结算',SETTLED:'已结算',CANCELLED:'已取消',REVERSED:'已调整',WAITING_CLAIM:'等待48小时结算',READ:'已读',UNREAD:'未读',EMPTY_NUMBER:'空号或停机',OUT_OF_SERVICE_REGION:'超出服务区域',DUPLICATE_TO_RECEIVER:'接收方重复客户',NON_HOUSING_CONSULTATION:'非建房装修咨询',ASSIGNED:'待处理',IN_PROGRESS:'核验中',SUPPORT_RETURN:'支持退回',DOES_NOT_SUPPORT_RETURN:'不支持退回',INCONCLUSIVE:'信息不足',UNCLAIMED_TIMEOUT:'未在领取截止前确认接收',CLAIM_TIMEOUT:'未在领取截止前确认接收',REFUSED_CLAIM:'接收方拒绝领取',RETURN_APPROVED:'退回审核通过',V12_RETURN_APPROVED:'退回审核通过'};
Object.assign(LABEL,{ACTIVE:'正常合作',REQUESTED:'已申请终止',TERMINATION_REQUESTED:'已申请终止',CLEARING:'清算中',NEED_MORE:'待补充',APPROVED_PENDING_PAYMENT:'待线下付款',PAID_PENDING_WRITE_OFF:'待确认核销',PAYMENT_FAILED:'付款失败',SETTLEMENT_ERROR:'核销异常',TERMINATED:'已终止客资合作'});
...
const VIEWS={home:['首页','home'],leads:['供资','plus'],points:['积分','coins'],notifications:['消息','bell'],profile:['我的','user'],assignments:['接收','hand-claim'],followups:['跟进','clipboard-check'],returns:['退回记录','rotate-ccw'],reports:['经营报表','chart-no-axes-combined'],rewards:['奖励','award'],termination:['终止客资合作','rotate-ccw']};
...
const canView=view=>{
  ...
  if(view==='termination')return can('supply.termination.own.manage')||can('supply.termination.own.read');
...
function profileBusinessEntries(){const entries=[];if(canView('returns'))entries.push(`<button class="wb-account-action" data-go="returns" type="button"><i>${icon('rotate-ccw')}</i><span><b>退回记录</b><small>查看退回原因、证据和审核进度</small></span><em>${icon('chevron-right')}</em></button>`);if(canView('reports'))entries.push(`<button class="wb-account-action" data-go="reports" type="button"><i>${icon('chart-no-axes-combined')}</i><span><b>经营报表</b><small>${isFranchiseOwner()?'查看公司汇总数据':'仅查看本人业务数据'}</small></span><em>${icon('chevron-right')}</em></button>`);if(canView('termination'))entries.push(`<button class="wb-account-action" data-go="termination" type="button"><i>${icon('rotate-ccw')}</i><span><b>终止客资合作</b><small>停止提供新客资，账号和历史记录继续保留</small></span><em>${icon('chevron-right')}</em></button>`);return entries.length?`<section class="wb-card wb-account-security"><div class="wb-card-head"><h2>业务记录</h2></div><div class="wb-account-actions">${entries.join('')}</div></section>`:''}
...
function ledgerLabel(type){return {RECHARGE:'充值入账',CLAIM:'领取扣分',RETURN:'退回返分',REWARD:'奖励到账',ADJUST:'人工调整',REVERSAL:'冲正调整',TERMINATION_WRITEOFF:'终止合作核销'}[type]||readableLabel(type,'积分变动')}
...
async function points(){
  ...
  const [account,customerLedgers,supplyLedgers,packages]=await Promise.all([api(`/points/accounts/${encodeURIComponent(companyId)}`),api(`/points/ledgers?company_id=${encodeURIComponent(companyId)}&point_kind=CUSTOMER&page=1&page_size=50`),api(`/points/ledgers?company_id=${encodeURIComponent(companyId)}&point_kind=SUPPLY&page=1&page_size=50`),api('/points/packages')]);
  const customerRows=customerLedgers.items||[],supplyRows=supplyLedgers.items||[];
  const delta=monthDelta(customerRows);
...
function terminationRecordIds(item){return (item.record_ids||[]).map(id=>recordCode(id,'JL')).join('、')}
function terminationBlockers(items){return (items||[]).map(item=>`<article class="wb-item"><div class="wb-item-top"><div><h3>${esc(item.label||TERMINATION_BLOCKER_LABEL[item.code]||'待处理事项')}</h3><p>${esc(item.message||item.description||'请完成相关业务后再次复检')}</p>${item.record_ids?.length?`<p>相关记录：${esc(terminationRecordIds(item))}</p>`:''}</div>${badge('PENDING','待处理')}</div></article>`).join('')||'<div class="wb-empty">当前没有清算阻断项</div>'}
function terminationSnapshot(request,state){if(!request)return'';return `<section class="wb-card"><div class="wb-card-head"><h2>结算快照</h2>${badge(request.status)}</div><div class="wb-detail-grid">${[['客资积分',request.customer_points_snapshot],['供客积分',request.supply_points_snapshot],['终止合作通用积分',request.general_points_snapshot],['兑换比例',`1 积分 = ${terminationMoney(request.cash_cents_per_point_snapshot)}`],['预计线下付款',terminationMoney(request.cash_amount_cents_snapshot)],['实际线下付款',request.payment_amount_cents==null?'--':terminationMoney(request.payment_amount_cents)],['收款方式',request.payment_method],['收款人',request.payee_name],['收款账号',request.payee_account||request.payee_account_masked],['付款流水号',request.payment_external_reference],['付款凭证地址',request.payment_proof_url],['付款时间',fmt(request.paid_at)]].map(([name,value])=>`<div class="wb-detail"><small>${esc(name)}</small><b>${esc(value??'--')}</b></div>`).join('')}</div><div class="wb-notice">线下付款确认后，系统分别核销本次冻结快照中的客资积分与供客积分；快照后新增客资积分保留。历史终止与付款记录继续保留。</div></section><section class="wb-card"><div class="wb-card-head"><h2>清算阻断项</h2></div><div class="wb-list">${terminationBlockers(request.blockers)}</div></section>`}
function terminationHistory(items){return `<section class="wb-card"><div class="wb-card-head"><h2>历史终止记录</h2></div><div class="wb-list">${(items||[]).map(item=>`<article class="wb-item"><div class="wb-item-top"><div><h3>${fmt(item.created_at)}</h3><p>终止合作通用积分：${esc(item.general_points_snapshot??'--')} · 结算金额：${esc(item.cash_amount_cents_snapshot==null?'--':terminationMoney(item.cash_amount_cents_snapshot))}</p></div>${badge(item.status)}</div></article>`).join('')||'<div class="wb-empty">暂无终止合作记录</div>'}</div></section>`}
function canCancelSupplyTermination(request){return ['REQUESTED','NEED_MORE','APPROVED_PENDING_PAYMENT','PAYMENT_FAILED'].includes(request?.status)}
function cancelSupplyTermination(){openSheet('付款前取消申请',`<form class="wb-form" id="termination-cancel-form"><div class="wb-notice">仅在平台登记线下付款前可以取消。取消后恢复提供新客资，历史申请仍保留。</div><div class="wb-field"><label for="termination-cancel-note">取消说明</label><textarea class="wb-input" id="termination-cancel-note" minlength="2" required></textarea></div><button class="wb-btn danger" id="termination-cancel-submit">确认取消申请</button></form>`,()=>{const form=document.querySelector('#termination-cancel-form'),submit=document.querySelector('#termination-cancel-submit');form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;try{await api('/v1.2/supply-termination/current/cancel',{method:'POST',body:JSON.stringify({note:document.querySelector('#termination-cancel-note').value.trim()})});closeSheet(form);toast('终止合作申请已取消');await supplyTermination()}catch(error){if(form.isConnected)submit.disabled=false;toast(error.message,true)}}})}
async function supplyTermination(){
  const state=await api('/v1.2/supply-termination/current');
  S.supplyTermination=state;
  const request=state.request||state.current_request||state.termination||(state.id?state:null);
  const canSubmit=can('supply.termination.own.manage')&&state.cooperation_status==='ACTIVE'&&(!request||['REJECTED','CANCELLED','TERMINATED'].includes(request.status));
  const form=canSubmit?`<section class="wb-card"><div class="wb-card-head"><div><h2>申请终止</h2><p>提交后停止新增供资，并进入清算和平台审核。</p></div></div><form class="wb-form" id="supply-termination-form"><div class="wb-field"><label for="termination-reason">终止原因</label><textarea class="wb-input" id="termination-reason" minlength="3" required></textarea></div><div class="wb-field"><label for="termination-payee-name">线下收款人</label><input class="wb-input" id="termination-payee-name" required></div><div class="wb-field"><label for="termination-payment-method">收款方式</label><select class="wb-select" id="termination-payment-method"><option value="BANK_TRANSFER">银行转账</option><option value="WECHAT_OFFLINE">线下微信转账</option><option value="ALIPAY_OFFLINE">线下支付宝转账</option><option value="OTHER">其他线下方式</option></select></div><div class="wb-field"><label for="termination-payee-account">收款账号</label><input class="wb-input" id="termination-payee-account" required></div><label class="wb-choice"><input type="checkbox" id="termination-confirm" required><span><b>确认终止合作范围</b><small>这是终止客资提供合作，不会停用公司或登录账号；接收、跟进和历史查询仍按现有权限使用。</small></span></label><button class="wb-btn danger" id="termination-submit" type="submit">提交终止客资合作申请</button></form></section>`:'';
  const cancelAction=canCancelSupplyTermination(request)?'<section class="wb-card"><div class="wb-actions"><button class="wb-btn danger" id="termination-cancel">付款前取消申请</button></div></section>':'';
  shell(`<section class="wb-page-head"><div><h1>终止客资合作</h1><span>${esc(readableLabel(state.cooperation_status,'正常合作'))}</span></div></section><div class="wb-notice">终止后不再提供新客资，公司和登录账号继续保留。客资积分与供客积分仅在终止合作结算时合并为终止合作通用积分，并由平台线下付款。</div>${terminationSnapshot(request,state)}${cancelAction}${form}${terminationHistory(state.history)}`);
  document.querySelector('#termination-cancel')?.addEventListener('click',cancelSupplyTermination);
  const terminationForm=document.querySelector('#supply-termination-form');
  if(terminationForm)terminationForm.onsubmit=async event=>{event.preventDefault();const button=document.querySelector('#termination-submit');if(!document.querySelector('#termination-confirm').checked){toast('请先确认终止合作范围',true);return}button.disabled=true;try{await api('/v1.2/supply-termination/requests',{method:'POST',body:JSON.stringify({reason:document.querySelector('#termination-reason').value.trim(),payee_name:document.querySelector('#termination-payee-name').value.trim(),payee_account:document.querySelector('#termination-payee-account').value.trim(),payment_method:document.querySelector('#termination-payment-method').value})});toast('终止合作申请已提交');await supplyTermination()}catch(error){if(button.isConnected)button.disabled=false;toast(error.message,true)}};
}
...
function supplyLeadActions(lead,canModify=true){
  if(!canModify)return `<button class="wb-btn" data-supply-detail="${esc(lead.id)}">查看进度</button>`;
...
async function leads(){
  ...
  const [page,capabilities,terminationState]=await Promise.all([api(`/v1.2/supplier/leads?${query}`),api('/v1.2/company/capabilities'),api('/v1.2/supply-termination/current')]);
  S.supplyTermination=terminationState;
  const supplierCapability=(capabilities||[]).find(item=>item.capability_code==='LEAD_SUPPLIER');
  const cooperationStopped=!supplyCooperationAllowsUpload(terminationState);
