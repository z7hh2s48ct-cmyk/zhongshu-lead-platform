...
async function rewards(){
  ...
  const sum=d.summary||{};
  const list=(d.items||[]).map((x,index)=>item(`序号 ${rowSequence(d,index)} · ${x.customer_name||'未填写客户'}`,x.status,`<p>${esc(x.lead_code||recordCode(x.lead_id,'KZ'))} · ${x.reward_points} 供客积分</p><p>当前进度：${esc(readableLabel(x.status))} · 预计结算：${fmt(x.reward_due_at)}</p>`,`<button class="wb-btn" data-reward="${x.id}">查看说明</button>`)).join('');
...
async function rewardDetail(id){
  ...
  openSheet('供客积分详情',`<div class="wb-detail-grid">${[['对应客资',`${x.customer_name||'未填写'} · ${x.lead_code||recordCode(x.lead_id,'KZ')}`],['当前进度',readableLabel(x.status)],['供客积分',x.reward_points],['对应客资积分',x.claim_points],amountRule,['开始观察',fmt(x.observed_at)],['交易确认条件时间',fmt(x.reward_due_at)],['实际到账',fmt(x.settled_at)]].map(([name,value])=>`<div class="wb-detail"><small>${esc(name)}</small><b>${esc(value??'--')}</b></div>`).join('')}</div><div class="wb-card"><h3>结算说明</h3><p class="wb-muted">${esc(rewardExplanation(x))}</p></div><div class="wb-notice">人工电话确认有效时及时结算；未提前确认的，领取后连续 48 小时未正式申请退回则自动结算入账。已入账奖励在退回审核通过时自动冲回，驳回则保持已入账。</div><div class="wb-actions"><button class="wb-btn" data-reward-lead="${esc(x.lead_id)}">查看对应客资</button></div>`,()=>document.querySelector('[data-reward-lead]')?.addEventListener('click',()=>{closeSheet();go('leads',x.lead_id)}),intent);
}
